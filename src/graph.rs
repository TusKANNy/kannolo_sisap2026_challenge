...
impl Graph {
    /// Rebuilds this graph (assumed to be the ground/level-0 graph) in the order given
    /// by `mapping` (`mapping[old_id] = new_id`), remapping every neighbor id accordingly.
    ///
    /// The returned graph has no `ids_mapping` (local id == new id, matching a dataset
    /// reordered with the same permutation). Also returns the inverse mapping
    /// (`new_id -> old_id`), which callers need to remap the dataset and to translate
    /// search results back to the original ids.
    pub fn remap_ground_with_permutation(&self, mapping: &[usize]) -> (Graph, Vec<usize>) {
        let n = self.n_nodes;
        assert_eq!(
            mapping.len(),
            n,
            "invalid graph permutation length: expected {n}, got {}",
            mapping.len()
        );
        crate::permutation::validate_permutation(mapping).unwrap_or_else(|e| panic!("{e}"));

        let inv = crate::permutation::invert_mapping(mapping);
        ...
        let graph = Graph {
            neighbors: all_neighbors.into_boxed_slice(),
    ...
    /// Rebuilds an upper HNSW level so that its local ids are ordered as
    /// `old_locals_by_new_local` (the `i`-th new local id corresponds to the old local
    /// id `old_locals_by_new_local[i]`), remapping each neighbor's local id accordingly
    /// and remapping `get_external_id` through `global_mapping` (`old_id -> new_id` for
    /// the ground level).
    ///
    /// Returns the remapped graph and `local_mapping` (`old_local -> new_local`).
    pub fn remap_level_with_old_local_order(
        &self,
        ...
        assert_eq!(
            ...
            n,
            "invalid upper-level local order length: expected {n}, got {}",
            old_locals_by_new_local.len()
        );
        ...
        let graph = Graph {
            neighbors: all_neighbors.into_boxed_slice(),
            offsets: new_offsets.into_boxed_slice(),
            ids_mapping: Some(new_external_ids.into_boxed_slice()),
            max_degree: self.max_degree,
            n_nodes: n,
        };
        (graph, local_mapping)
