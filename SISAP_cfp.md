...
Examples
Final comments
📢 Submission Details Now Available
The submission details for the SISAP 2026 Indexing Challenge are now available.

Please see the submission details and evaluation page for instructions and requirements.
...
×
k
n×k matrix of object identifiers (integers), where 
n
n is the number of queries and 
k
k is the number of neighbors. The 
i
i-th row contains the identifiers of the 
k
k nearest neighbors of the 
i
i-th query. Identifiers must use 1-based indexing (i.e., the first object in the dataset has ID 1).

dists: An 
n
×
k
n×k matrix of distances (floats). The 
i
i-th row contains the distances of the 
k
k nearest neighbors of the 
i
i-th query according your implementation.
...
Registration and Participation
To facilitate running the challenge, please register for the challenge by opening a "Pre-registration request" issue in the GitHub repository https://github.com/sisap-challenges/challenge2026/. Fill out the required data, taking into account that the given data will be used to keep in contact while the challenge remains open. We use this system to keep track of potential participants; for later registration, contact the organizers first.

During the development phase, participants will have access to gold standards for all tasks.

Teams submit their solutions through TIRA. Submissions are required to run in Docker containers and results have to be written in a standard format to unify the evaluation. Please see the submission details and evaluation page for the current workflow and examples.

TIRA submissions will be tested at the time of the challenge. Results will be shared with the authors for verification and potential fixes before the final rankings are published. The short paper that is to be submitted following an entry will be submitted before the final rankings are published and should thus focus on a self-evaluation of the proposed system.

The private workloads that are used in the evaluation are shared publicly after the evaluation has been carried out.

One person can only be part of a single team.
